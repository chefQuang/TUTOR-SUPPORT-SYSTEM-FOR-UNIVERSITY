...
interface Session {
  ...
  courseName: string; classId: string; isCompleted: boolean; type?: 'Teaching' | 'Consultation' | 'Learning';
}
...
const fetchSchedule = async () => {
  try {
    ...
    if (res.data.success) {
       // NẾU CODE CŨ LÀ setSessions(res.data.data) THÌ ĐÚNG RỒI.
       // NHƯNG CÓ THỂ DO LOGIC BACKEND TRẢ VỀ BỊ DUPLICATE.
  ...
  const handleUpdateSession = async () => {
    ...
    const dateToSend = newDateStr || selectedSession.date;
    if (!newDateStr) {
    ...
    try {
      const res = await axios.post('http://localhost:5000/api/tutor/update-session', {
        tutorId: user.id,
        sessionId: selectedSession?.id,
        newDate: dateToSend, // Dùng ngày mới chọn
      ...
      if (res.data.success) {
        alert(res.data.message); // Hiện thông báo từ server
        setShowModal(false);     // Đóng modal
        fetchSchedule();         // Load lại lịch
        setSelectedSession(null); // Bỏ chọn
  ...
  const handleToggleStatus = async () => {
    if (!selectedSession) return;
    const newStatus = !selectedSession.isCompleted; // Đảo ngược trạng thái hiện tại

    try {
      const res = await axios.post('http://localhost:5000/api/tutor/toggle-status', {
        tutorId: user.id,
        sessionId: selectedSession.id,
        status: newStatus
      });
      if (res.data.success) {
        ...
        fetchSchedule(); // Reload lại dữ liệu mới
        setSelectedSession(null); // Reset selection
      }
    } catch (err: any) {
      alert("Cannot change status for this session type (e.g. Consultations are auto-only).");
    }
  };
  ...
  const dailySessions = sessions
    ...
    // Thêm logic khử trùng lặp dựa trên ID
    .filter((v, i, a)=> a.findIndex(t => (t.id === v.id && t.classId === v.classId)) === i
    );
      ...
      <div className="schedule-container">
        ...
        <div className="details-panel" style={{display:'flex', flexDirection:'column'}}>
          <div className="selected-date-title">
            {/* FIX LỖI NGÀY: Dùng toDateString để hiển thị đúng ngày local */}
          ...
          <div style={{display:'flex', flexDirection:'column', gap:'10px', marginBottom:'20px', paddingBottom:'20px', borderBottom:'1px solid #eee'}}>
             <button className="btn-action" style={{background:'#1e293b'}} onClick={() => alert("Feature coming soon")}>+ Add Time Slot</button>
             
             {selectedSession ? (
               <>
                 <button className="btn-action" style={{background:'white', color:'#334155', border:'1px solid #ccc'}} onClick={() => setShowModal(true)}>
                    ✏️ Change Time
                 </button>
                 
                 {/* NÚT TOGGLE STATUS MỚI */}
                 <button 
                    className="btn-action" 
                    style={{
                        background: selectedSession.isCompleted ? '#f0fdf4' : '#fffbeb', 
                        color: selectedSession.isCompleted ? '#166534' : '#b45309', 
                        border: selectedSession.isCompleted ? '1px solid #bbf7d0' : '1px solid #fde68a'
                    }} 
                    onClick={handleToggleStatus}
                 >
                    {selectedSession.isCompleted ? '↩️ Mark as Upcoming' : '✅ Mark as Completed'}
                 </button>

                 <button className="btn-action" style={{background:'white', color:'#dc2626', border:'1px solid #fecaca'}} onClick={handleCancelSession}>
                    ❌ Cancel Slot
                 </button>
               </>
             ) : (
          ...
          <div style={{overflowY:'auto', flex:1}}>
            {dailySessions.length > 0 ? dailySessions.map(sess => (
              <div 
                key={`${sess.classId}-${sess.id}`}
                // Thêm class 'completed' để đổi màu xám nếu đã xong
                className={`session-card ${selectedSession?.id === sess.id && selectedSession?.classId === sess.classId ? 'active' : ''} ${sess.isCompleted ? 'completed' : 'upcoming'}`}
                onClick={() => handleSelectSession(sess)}
              >
                {/* Badge Status */}
                <span className={`status-badge ${sess.isCompleted ? 'completed' : 'upcoming'}`} style={{position:'absolute', top:'10px', right:'10px'}}>
                  {sess.isCompleted ? 'Completed' : 'Upcoming'}
                </span>

                <div className="session-time">🕒 {sess.time}</div>
                <div className="session-course">{sess.courseName}</div>
                <div className="session-info">
                    {sess.type === 'Consultation' ? `Student: ${sess.classId}` : `Class: ${sess.classId}`}
                </div>
              </div>
